...
class GA1Calibration:
    ...
    def _init_population(self) -> np.ndarray:
        ...
        for _ in range(self.pop_size):
            parameters.append(
                [
                    np.random.uniform(self.L_p[i], self.U_p[i])
    ...
    def _crossover(self, parents):
        ...
        chromosome_1[gene] = chromosome_1[gene] * ratio + chromosome_2[gene] * (
            1 - ratio
        )
        chromosome_2[gene] = chromosome_2[gene] * ratio + chromosome_1[gene] * (
            1 - ratio
        )
    ...
    def run(self) -> None:
            ...
            for chr_id in range(len(new_population)):
                for gene_id in range(self.num_of_params):
                    if np.random.random() < self.p_m:
                        new_value = new_population[chr_id][gene_id] + np.random.normal(
                            0, self.mutation_range
                        )
                        new_population[chr_id][gene_id] = np.clip(
                            new_value, self.L_p[gene_id], self.U_p[gene_id]
                        )
        ...
        self.prediction_error = np.abs(
            np.array([self.d_real, self.mu_real]) - self.result[np.argmax(self.fitness)]
        )
    ...
    def plot_best(self):
        ...
        plt.plot(self.best)
        plt.axhline(
            y=self.stop_fitness, color="r", linestyle="--", label="Stop Fitness"
        )
        ...
        plt.legend()
        plt.savefig(
            f"results/GA1_calibration_{self.name}.png", dpi=300, bbox_inches="tight"
        )
        plt.close()
...
class GA2Calibration:
    ...
    def plot_best(self):
        ...
        plt.legend()
        plt.savefig(
            f"results/GA2_calibration_{self.name}.png", dpi=300, bbox_inches="tight"
        )
        plt.close()
